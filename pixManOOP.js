...
function over(number) {
    ...
    let outDivText = "Equation:<br>PixNum = y*width+x<br>"
    outDivText += `x = ${number%w}<br>`
    outDivText += `y = ${Math.floor(number/w)}<br>`
    outDivText += `PixNum = ${Math.floor(number/w)}*${w}+${number%w}<br>`
    outDivText += `PixNum = ${number}<br>`
